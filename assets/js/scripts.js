...
!function (NioApp, $) {
    ...
    NioApp.CurrentLink = function () {
        ...
        $(_link).each(function () {
            var self = $(this),
                _self_link = getLocation(self.attr('href')).pathname;

            if (_self_link == currentPath) {
    ...
    NioApp.Toast = function (msg, ttype, opt) {
        var ttype = ttype ? ttype : 'info',
            msi = '',
            ticon = ttype === 'info' ? 'fas fa-info-fill' : ttype === 'success' ? 'fas fa-check-circle-fill' : ttype === 'error' ? 'fas fa-cross-circle-fill' : ttype === 'warning' ? 'fas fa-alert-fill' : '',
            def = {
        ...
        msi = attr.icon !== '' ? '<span class="toastr-icon"><i class="icon ' + attr.icon + '"></i></span>' : '', msg = msg !== '' ? msi + '<div class="toastr-text">' + msg + '</div>' : '';
    ...
    NioApp.ModeSwitch = function () {
        ...
        toggle.on('click', function (e) {
            ...
            localStorage.setItem('dark_mode', $(this).hasClass('active'));
        });

        if (localStorage.getItem('dark_mode') == 'true') $('.dark-switch').trigger('click');
    }; // Knob @v1.0
    ...
    NioApp.Slick = function (elm, opt) {
        if ($(elm).exists() && typeof $.fn.slick === 'function') {
            $(elm).each(function () {
                var def = {
                        'prevArrow': '<div class="slick-arrow-prev"><a href="javascript:void(0);" class="slick-prev"><i class="icon fas fa-chevron-left"></i></a></div>',
                        'nextArrow': '<div class="slick-arrow-next"><a href="javascript:void(0);" class="slick-next"><i class="icon fas fa-chevron-right"></i></a></div>',
