...
$ManhDev->xoa("soucerWeb", "`id` = '$id' ");
echo '<script type="text/javascript">location.href = "/admin/createWeb";</script>';
}
...
$ManhDev->xoa("history_createWeb", "`id` = '$id' ");
echo '<script type="text/javascript">location.href = "/admin/createWeb";</script>';
}
...
$ManhDev->update("history_createWeb", [
            'status' => 'hoantat',
            'note_giahan' => "Duyệt Từ ".$time1." Thời Gian Hết Hạn Sẽ Là ".date('d-m-Y', $date),
            'end' => $date
            ], " `id` = '$id' ");
echo '<script type="text/javascript">location.href = "/admin/createWeb";</script>';
}
...
<?php 
  $i = 1;
  $user = $ManhDev->query("SELECT * FROM `soucerWeb` ORDER BY `id` DESC ");
...
echo '<script>setTimeout(function(){ location.href = "" },2000); Swal.fire("Thông Báo", "Thêm Loại Web Bán Thành Công", "success");</script>';
}
}
?>
...
<section class="col-lg-12 connectedSortable">
            <div class="card card-info">
                <div class="card-header">
                    <h3 class="card-title">DANH SÁCH WEB KHÁCH TẠO</h3>
                </div>
                <div class="card-body">
                    <div class="card-body table-responsive p-0">
                        <table id="datatable1" class="table table-bordered table-striped">
...
<?php 
  $i = 1;
  $user = $ManhDev->query("SELECT * FROM `history_createWeb` ORDER BY `id` DESC ");
  while ($row1 = mysqli_fetch_array($user)) { ?>
                                <tr class="text-center">
                                    <td><?=$i++;?></td>
                                    <td><?=$row1['username'];?></td>
                                    ...
                                    <td><?=$row1['title'];?></td>
                                    <td><?=$row1['domain'];?></td>
                                    <td><?=$row1['timemua'];?> Tháng</td>
                                    <td><?=tien($row1['money']);?></td>
                                    <td><?=taoweb($row1['status']);?></td>
                                    <td><?=$row1['time'];?></td>
                                    <td><?=date('d-m-Y', $row1['end']);?></td>
                                    <td><?=$row1['note_giahan'];?></td>
                                    <td>
                                        <?php if($row1['status'] == "xuly") { ?>
                                        <a href="?duyet=<?=$row1['id'];?>" class="btn btn-success btn-sm">Duyệt</a>
                                        <?php } ?>
                                        <a href="?xoa_list=<?=$row1['id'];?>" class="btn btn-danger btn-sm">Xóa</a>
                                    </td>
                                </tr>
                                <?php }?>
                            </tbody>
